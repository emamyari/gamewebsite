...
def page2(request):
    data = requests.get(
        'https://api.digikala.com/v1/promotions/plp_63247410/?has_selling_stock=1&sort=7&seo_url=&page=2').text
    d = json.loads(data)
...
def products(request):
    data = requests.get(
        'https://api.digikala.com/v1/categories/wearable-gadget/search/?brands%5B0%5D=10&camCode=582&seo_url=%2Fcategory-wearable-gadget%2F%3Fbrands%255B0%255D%3D10%26camCode%3D582&page=1').text
    d = json.loads(data)
...
def register(request):
    if request.method == 'POST':
        ...
        made = request.data['madein']
        count = request.data['count']

        connection = pyodbc.connect(
            'DRIVER={ODBC DRIVER 17 FOR SQL SERVER};Server=192.168.1.56;Database=emamyari;Port=1433;UID=sa;PWD=111')
        cursor = connection.cursor()
        cursor.execute('insert into products(title,color,price,made,weight,count)values(?,?,?,?,?,?)'
                       , name, color, price, made, w,count)
        cursor.commit()
